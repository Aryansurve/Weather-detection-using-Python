...
class Weather(Tk):
    ...
    def __gui(self):
       ...
       Button(text='Graph',font=("Georgia",16,"bold"),bg='orange',fg='black',width=7,relief='groove',activebackground="blue",activeforeground='white',command=self.open).place(x=440,y=420)
    ...
    def open(self):
        ...
        # Create a dropdown list of cities
        cities = ['Mumbai', 'Delhi', 'Bangalore', 'Hyderabad', 'Kolkata']

        selected_city = StringVar()
        selected_city.set(cities[0])  # Set the default selected city

        # city_dropdown = OptionMenu(graph_window, selected_city, *cities)
        # city_dropdown.config(font=("Segoe UI", 12), width=15)
        # city_dropdown.pack(pady=10)

        city_dropdown = ttk.Combobox(graph_window, textvariable=selected_city, values=cities)
        ...
        def temp(city):
            if city == 'Mumbai':
                temperatures = [30, 32, 33, 34, 32, 31, 30]
            elif city == 'Delhi':
                temperatures = [25, 27, 28, 29, 26, 25, 24]
            elif city == 'Bangalore':
                temperatures = [28, 29, 30, 28, 27, 26, 25]
            elif city == 'Hyderabad':
                temperatures = [32, 33, 34, 33, 32, 31, 30]
            elif city == 'Kolkata':
                temperatures = [31, 32, 33, 32, 30, 29, 28]
            else:
                temperatures = []
            return temperatures
        def plot_graph(city):
            # Get the temperature data for the previous 7 days
            temperatures = temp(city)
            # Plot the graph
            plt.clf()
            # Get the dates for the previous 7 days
            dates = [(datetime.datetime.now() - datetime.timedelta(days=i)).strftime('%d/%m/%Y') for i in range(1, 8)]
            plt.plot(dates, temperatures)
            plt.xlabel("Day")
            plt.ylabel("Temperature (°C)")
            plt.title(f"Temperature Graph for {city}")
            plt.grid(True)
            canvas.draw()

        # Create a button to plot the graph
        update_button = ttk.Button(graph_window, text="Update Graph", command=lambda: plot_graph(selected_city.get()))
        update_button.grid(row=0, column=1, padx=10, pady=10)
        canvas = FigureCanvasTkAgg(plt.figure(figsize=(8, 6)), master=graph_window)
        canvas.get_tk_widget().grid(row=1, columnspan=2, padx=10, pady=10)

        plot_graph(selected_city.get())
